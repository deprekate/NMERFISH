...
#activate cellpose&&python D:\Scripts\NMERFISH\workerSoniaComb2Mouse.py

master_analysis_folder = r'C:\Scripts\NMERFISH'
lib_fl = r'C:\Scripts\NMERFISH\codebooks\blank_codebook_2colDNA_Van_overlap.csv'
### Did you compute PSF and median flat field images?
psf_file = master_analysis_folder+r'\psfs\psf_750_Scope4_final.npy'
master_data_folder = r'\\192.168.0.127\Raw_data\Van\6_12_2023__VanReview_DropletHeartMI_S2'
...
def compute_fits(save_folder,fov,all_flds,redo=False,ncols=4,
                psf_file = psf_file,try_mode=True,try_read=False):
    ...
    for fld in tqdm(all_flds):
        for icol in range(ncols-1):
            ...
            if not os.path.exists(save_fl) or redo:
                redo_this=True
            if try_read and os.path.exists(save_fl) and (not redo_this):
                try:
...
def compute_decoding(save_folder,fov,set_,redo=False):
    ...
    if complete==0 or redo:
        dec.get_XH(fov,set_,ncols=3,nbits=20)#number of colors match 
        ...
        dec.get_icodes(n_on_bits=2,method = 'top4',norm_brightness=None,nbits=60)
...
def get_files(set_ifov):
    ...
    all_flds = glob.glob(master_folder+r'\H*_MER*')
    all_flds += glob.glob(master_folder+r'\H*Q*')
...
def compute_main_f(save_folder,all_flds,fov,set_,ifov,try_read,redo_fits,redo_drift,redo_decoding,try_mode):
    print("Computing fitting on: "+str(fov))
    print(len(all_flds),all_flds)
    compute_fits(save_folder,fov,all_flds,try_read=try_read,redo=redo_fits,try_mode=try_mode)
...
def main_f(set_ifov,try_read=False,redo_fits = False,redo_drift=False,redo_decoding=False,try_mode=True):
    set_,ifov = set_ifov
    save_folder,all_flds,fov = get_files(set_ifov)
    
    if try_mode:
        try:
            compute_main_f(save_folder,all_flds,fov,set_,ifov,try_read,redo_fits,redo_drift,redo_decoding,try_mode)
        except:
